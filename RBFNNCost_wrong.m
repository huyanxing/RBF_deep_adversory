function [cost,grad] = RBFNNCost_wrong(theta, visibleSize, hiddenSize,numClasses,lambda, features_total, labels_total, settings, num_minibatch)
%%                                                         
% Instructions:
...
if exist('num_minibatch','var')
    maxepoach = num_minibatch;
else 
    maxepoach = 1;
end 
...
sample_num = size(features_total,2);
max_batchSize = ceil(sample_num / maxepoach);
sizeMaker = zeros(1,maxepoach);
for i = 1:maxepoach
    if i*max_batchSize <=sample_num
        sizeMaker(i) = i*max_batchSize;
    else
        sizeMaker (i)= sample_num;
    end
end

groundTruth_total = full(sparse(labels_total, 1:sample_num, 1));
cost = 0;

%% start mini-batch
for epoach = 1 : maxepoach
    if epoach  == 1
         features = features_total(:,1:sizeMaker (epoach) );
         %labels = labels_total(1:sizeMaker (epoach));
         groundTruth = groundTruth_total(:,1:sizeMaker (epoach));
         batchSize = max_batchSize;
    else
        features = features_total(:,sizeMaker(epoach-1)+1:sizeMaker (epoach) );
        %labels = labels_total(sizeMaker(epoach-1)+1:sizeMaker (epoach) );
        groundTruth = groundTruth_total(:,sizeMaker(epoach-1)+1:sizeMaker (epoach) );
        batchSize = sizeMaker (epoach) - sizeMaker(epoach-1);
    end
 %% --------disp([numgrad grad]);    
...
% errorterm in layer3 from data L-2 norm
    ...
    W2grad = (1/batchSize).*W2grad + lambda*W2;

    b2grad = b2grad+sum(errorterm_3,2);
    b2grad = (1/batchSize)*b2grad;

    reg_grad =beta*(-sparsityParam./rho+(1-sparsityParam)./(1-rho));
    errorterm_2 = W2'*errorterm_3.*a2 + repmat(reg_grad,1,batchSize).*a2;
    ...
    centroidsgrad = (1/batchSize).*centroidsgrad + lambda*centroids;

    if isnumeric(sigmavalue) == 0
        sigmagrad = sigmagrad + sigmagrad_Update;
        sigmagrad = (1/batchSize).*sigmagrad;
    end
end
cost = cost/maxepoach;
if isnumeric(sigmavalue) == 0    
